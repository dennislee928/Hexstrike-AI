...
export default function GAUPage() {
  ...
  const scanProfiles = [
    { id: 'standard', name: 'Standard Search', description: 'Find subdomains', estimatedTime: '2-5 minutes', config: {} }
  ]
  ...
  const handleScan = async () => {
    setIsRunning(true)
    try {
      const response = await fetch('https://hexstrike-ai.dennisleehappy.org/api/tools/gau', {
        method: 'POST',
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-cyber font-bold text-neon-blue neon-glow">GAU</h1>
          <p className="text-cyber-light font-mono text-sm mt-1">Subdomain finder</p>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <ToolForm title="Configuration" subtitle="Configure GAU" icon={<Search className="h-5 w-5 text-neon-blue" />} isRunning={isRunning} onSubmit={handleScan}>
