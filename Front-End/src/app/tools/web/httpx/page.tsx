...
export default function HTTPxPage() {
  ...
  const scanProfiles = [
    { id: 'standard', name: 'Standard Probe', description: 'HTTP probe', estimatedTime: '1-3 minutes', config: {} }
  ]
  ...
  const handleScan = async () => {
    setIsRunning(true)
    try {
      const response = await fetch('https://hexstrike-ai.dennisleehappy.org/api/tools/httpx', {
        method: 'POST',
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-cyber font-bold text-neon-blue neon-glow">HTTPX</h1>
          <p className="text-cyber-light font-mono text-sm mt-1">HTTP probe</p>
        </div>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <ToolForm title="Configuration" subtitle="Configure HTTPx" icon={<Activity className="h-5 w-5 text-neon-blue" />} isRunning={isRunning} onSubmit={handleScan}>
            <FormField label="Target" type="text" value={target} onChange={setTarget} placeholder="example.com" required />
          </ToolForm>
